...
func _execute_skill(skill_id: String, data: Dictionary, t_pos: Vector3, indicator: Node = null):
	...
	if skill_id == "heal":
		var actual_heal = min(dmg + int(magic_attack * 0.5), max_health - current_health)
		if actual_heal < 0: actual_heal = 0
		restore_hp(dmg + int(magic_attack * 0.5))
		spawn_floating_text("+" + str(actual_heal) + " HP", Color(0.2, 1, 0.2))
		var flash_tween = get_tree().create_tween()
		modulate = Color(2.0, 2.0, 2.0)
		flash_tween.tween_property(self, "scale", Color(1.0, 1.0, 1.0), 0.3)
		...
		var final_dmg = int((magic_attack * 1.2 + dmg) * el_multiplier)
		var elements = ["udara"]
		...
		var final_dmg = int((magic_attack * 1.8 + dmg) * el_multiplier)
		var elements = ["tanah"]
		...
		var final_shield = int((magic_attack * 2.0 + dmg) * el_multiplier)
		if status_manager:
			status_manager.apply_effect("holy_veil", dur, {"shield": final_shield})
		...
		spin_bar = ProgressBar.new()
		spin_bar.min_value = 0
		spin_bar.max_value = max_spin_time
		spin_bar.value = max_spin_time
		spin_bar.show_percentage = false
		spin_bar.custom_minimum_size = Vector2(30, 4)
		spin_bar.position = Vector2(-15, 12)
		...
		stab_tween.tween_property(self, "global_position", original_pos, 0.15).set_ease(Tween.EASE_IN).set_delay(0.1)
		stab_tween.tween_callback(func(): 
		...
		attack(false) # Start animation first
		
		var wave_dmg = dmg
		var p_attack = physical_attack
		var p_pos = global_position
		...
		if status_manager:
			status_manager.apply_effect("endure", dur)
		...
		if is_instance_valid(indicator) and indicator.get("single_target_node") != null and is_instance_valid(indicator.single_target_node):
			...
			if enemy.get("status_manager"):
				...
				spawn_floating_text("Target Marked!", Color(1.0, 0.2, 0.2))
		...
		if target_enemy == null:
			spawn_floating_text("No target!", Color(1, 0.5, 0))
			return
		
		var proj_scene = load("res://Scenes/Skills/player_projectile.tscn")
		if proj_scene:
			falcon_dive_active = true
			
			for i in range(4):
				if not is_instance_valid(target_enemy) or target_enemy.get("is_dead") or is_dead: break
				
				# Update animation facing direction
				last_direction = (target_enemy.global_position - global_position).normalized()
				var anim_dir = Vector2(last_direction.x, -last_direction.z)
				...
				if animation_tree:
					animation_tree.set("parameters/Attack/blend_position", anim_dir)
					
				if state_machine: state_machine.travel("Attack")
				apply_camera_shake(3.0, 0.1) # Ringan
			...
			if is_instance_valid(target_enemy) and not target_enemy.get("is_dead") and not is_dead:
				Engine.time_scale = 0.3
				
				var tween = get_tree().create_tween().set_ignore_time_scale(true)
				tween.tween_property(sprite, "position:y", base_y_offset - 0.4, 0.1).set_ease(Tween.EASE_OUT)
				tween.tween_property(sprite, "position:y", base_y_offset, 0.1).set_ease(Tween.EASE_IN)
				...
				Engine.time_scale = 1.0
				apply_camera_shake(6.0, 0.2) # Normal shake
		...
		if scene:
			...
			var tween = get_tree().create_tween().set_ignore_time_scale(true).set_parallel(true)
			tween.tween_property(sprite, "position:y", base_y_offset - 0.5, 0.2).set_ease(Tween.EASE_OUT)
			tween.tween_property(sprite, "rotation:y", deg_to_rad(360), 0.4)
			tween.chain().tween_property(sprite, "position:y", base_y_offset, 0.2).set_ease(Tween.EASE_IN)
			tween.parallel().tween_property(sprite, "rotation:y", deg_to_rad(0), 0.0)
			...
			inst.elements = atk_elements
			get_tree().current_scene.add_child(inst)
	elif skill_id == "haste":
		if status_manager:
			status_manager.apply_effect("haste", dur)
			spawn_floating_text("Haste!", Color(1.0, 1.0, 0.5))
			
	elif skill_id == "mirage_strike":
		mirage_strike_charges = 3
		if status_manager:
			status_manager.apply_effect("mirage_strike", dur)
		spawn_floating_text("Mirage Strike!", Color(0.5, 0.8, 1.0))
		
	elif skill_id == "poison_weapon":
		if status_manager:
			status_manager.apply_effect("poison_weapon", dur)
		spawn_floating_text("Poison Weapon!", Color(0.3, 0.8, 0.3))
			

	elif skill_id == "shadow_walk":
		if status_manager:
			status_manager.apply_effect("shadow_walk", dur)
		spawn_floating_text("Shadow Walk", Color(0.3, 0.3, 0.3))
		
	elif skill_id == "thief":
		if not status_manager.has_effect("shadow_walk"):
			spawn_floating_text("Harus Stealth!", Color(1, 0.5, 0))
		else:
			var enemies = get_tree().get_nodes_in_group("Enemy")
			var stole = false
			for e in enemies:
				if global_position.distance_to(e.global_position) < 0.5:
					if e.has_method("drop_loot"):
						e.drop_loot()
					spawn_floating_text("Item Stolen!", Color(1, 1, 0))
					stole = true
					break
			if not stole:
				spawn_floating_text("Missed!", Color(0.5, 0.5, 0.5))
		...
		if target_enemy == null:
			...
			global_position = target_enemy.global_position + dir * 0.3
			target_enemy.take_damage(dmg * 2, global_position, atk_elements)
			spawn_floating_text("Phantom Strike!", Color(0.6, 0.2, 0.8))
		...
		if target_enemy == null:
			...
			for i in range(10):
				...
				global_position = target_enemy.global_position + offset
				target_enemy.take_damage(int(dmg * 0.1), global_position, atk_elements)
